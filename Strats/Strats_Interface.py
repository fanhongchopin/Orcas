...
class RTD_settings(Tkinter.Frame):
	def __init__(self, master=None):
		...
		self.label_RTD_Settings = Tkinter.Label(self.line_1_frame, text = u"*********** 样本截面数据统计描述参数设置 ***********",bg= 'yellow')
		self.label_RTD_Settings.pack(side = 'top',anchor = 'w',expand = 'yes',fill = 'x')
	
		self.line_2_frame = Tkinter.Frame(self.frame) 
		self.line_2_frame.pack(side = 'top',expand = 'yes',fill = 'x')
		self.label_RT_Dir = Tkinter.Label(self.line_2_frame, text = u'样本数据文档路径:',width = 25,anchor = 'w')
		self.label_RT_Dir.pack(side = 'left',anchor = 'w',expand = 'yes',fill = 'x')
		self.text_RT_Dir = Tkinter.Text(self.line_2_frame, height = 1, width = 36)
		self.text_RT_Dir.pack(side = 'left',anchor = 'w',expand = 'yes',fill = 'x')
		self.text_RT_Dir.insert(END,Config.ctc_loan_tape)

	def Run_RTD(self, open_in_html_IN = False):
		temp_rtd_txt = Config.temp_rtd_txt
		rt_txt = self.text_RT_Dir.get("1.0","end-1c")
