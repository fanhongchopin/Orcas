...
class cashloan_ctc_mode(object):
	...
	def run_cashflow(self):
		...
		for idx,row in self.static_pool.iterrows():
			...
			cf_df = pd.DataFrame(index = range(0,total_term+1),columns = ['datetime','period','bop_bal','schedule_pmt','schedule_prin','schedule_int','default','prepay','expected_recovery','loss',
																		  'recovery','recovery_bal_new','recovery_bal','prin_cf','int_cf','total_cf','eop_bal']).fillna(0)
			cf_df.loc[0,'datetime'] = pd.to_datetime(loan_origination_date)
			datetime = loan_origination_date
			timedelta_45_day = pd.Timedelta(45,unit = 'd')
			timedelta_1_month = pd.Timedelta(1,unit = 'M')
			...
			for period_i in range(1,total_term+1):
				...
				if period_i == 1:
					datetime = datetime + timedelta_45_day
				else:
					datetime = datetime + timedelta_1_month
